...
$statement = $pdo->prepare("SELECT * FROM tbl_agent WHERE agent_email=? AND agent_token=?");
...
if(isset($_POST['form1'])) {
	...
	if( empty($_POST['agent_new_password']) || empty($_POST['agent_re_password']) )
	{
		$valid = 0;
	...
	{
		if($_POST['agent_new_password'] != $_POST['agent_re_password'])
        {
            $valid = 0;
	...
	if($valid == 1) {
		$statement = $pdo->prepare("UPDATE tbl_agent SET agent_password=?, agent_token=?, agent_time=? WHERE agent_email=?");
		$statement->execute(array(md5($_POST['agent_new_password']),'','',$_GET['email']));
						...
						<form action="" method="post">
							<div class="form-row">
								<div class="form-group">
									...
									<input type="password" class="form-control" name="agent_new_password" placeholder="New Password">
								</div>
								<div class="form-group">
									<label for="">Confirm Password</label>
									<input type="password" class="form-control" name="agent_re_password" placeholder="Confirm Password">
								</div>
